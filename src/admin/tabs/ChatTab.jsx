...
function ApprovalCard({ request, onRefresh }) {
  ...
  return (
    <div className="rounded-xl p-3 bg-pink-500/[0.06] border border-pink-500/20">
      <p
        className="text-pink-400 font-semibold uppercase mb-1"
      ...
      <p className="text-[#4a4565] mt-0.5" style={{ fontFamily: 'var(--font-mono)', fontSize: '10px' }}>
...
export default function ChatTab({ pendingRequests, onRefreshRequests }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: 'ai', text: data.response, intent: data.intent }]);
    } catch {
  ...
  return (
    <div className="flex flex-col h-full">
      <div ref={listRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              ...
              style={{ fontFamily: msg.role === 'ai' ? 'var(--font-mono)' : 'var(--font-display)' }}
            >
              {msg.intent && (
                <span
                  className="inline-block px-1.5 py-0.5 bg-purple-500/15 text-purple-400 rounded border border-purple-500/20 mb-1 mr-1 uppercase"
        ...
        {sending && (
          <div className="flex justify-start">
            <div className="bg-card text-purple-400 px-3 py-2 rounded-xl border border-purple-500/10">
      ...
      <div className="p-3 border-t border-purple-500/10 bg-deep shrink-0">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2.5 bg-surface rounded-xl text-sm text-white placeholder-[#4a4565] outline-none focus:ring-2 focus:ring-purple-500/50 border border-purple-500/10"
            style={{ fontFamily: 'var(--font-mono)' }}
          />
          <button
            onClick={send}
